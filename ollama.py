...
class AppConfig:
   ...
   def parse_numero_processo(numero: str) -> tuple[str, str, str, str, _]:
   ...
   def try_save_debug_storage_state(context) -> None:
       ...
       try:
           page.expect_popup(timeout=20000) as popup_info:
               page.expect_popup(timeout=20000)
               popup_page = popup_info.value
               popup_page.wait_for_load_state("networkidle", timeout=60000)
               return
       except (PlaywrightTimeoutError, PlaywrightError) as exc:
           last_exc = exc
           log_message(f"Tentativa {attempt} de dismissar o popup de certificado falhou: {exc}")
           try:
               page.expect_popup(timeout=20000)
    ...
    def go_to_consulta_via_processo_link(page) -> None:
        ...
        if processo_link.count() == 0:
            ...
            processo_link = page.locator("a[href='/pje/Processo/ConsultaProcesso/listView.seam']").first

        processo_link.wait_for(state="visible", timeout=60000)

        titulo = process_link.get_attribute("title") or "(sem título) "
   ...
   def download_processo_pdf(detail_page) -> Path:
       ...
       for attempt in range(1, 4):
           try:
               menu_download.wait_for(state="visible", timeout=20000)
